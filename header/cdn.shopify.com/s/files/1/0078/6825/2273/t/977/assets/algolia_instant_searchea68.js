...
//   ]);
    ...
    return item.refinements
      .map(function(refinement) {
        return (
          ...
          facet.title +
          '      </div>: ' +
          refinement.label.replace('vuejs_changeover_product_', '') +
          '    </div>' +
          '  </a>' +
          '</li>'
        );
  ...
  instant.search.addWidgets([
    hits({
      ...
      transformItems: function(products, results) {
        let filteredProducts = [];
        filteredProducts = products.reduce(function (acc, product) {
          const found = acc.find(item => item.id === product.id) 
          if (found) {
            found.variants.push({
              title: product.option1, variant_id: product.objectID , color: null, can_order:
        ...
        const mappedProducts = filteredProducts.map(function(product) {
        ...
        window.filteredProducts = mappedProducts;
        if (!window.alInitialFetch) {
          let ev = new CustomEvent("resultsFound");
